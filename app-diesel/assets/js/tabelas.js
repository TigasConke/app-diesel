 ...
 if (window.location.pathname.includes('tabela-cliente.html')) {
    ...
    if (window.location.pathname.includes('tabela-os.html')) {
        ...
        const renderTablePage = () => {
            ...
            pageOs.forEach(os => {
                const statusClass = os.status === 'Em andamento' ? 'status-em-andamento' : 'status-concluido';
                const servicos = os.servicos.map(s => s.descricao).join(', ') || 'N/A';
                
                const row = `
                    <tr>
                        <td>${servicos}</td>
                        <td>${os.propriedade?.cliente?.nome || 'N/A'}</td>
                        <td>${os.responsavel?.nome || 'N/A'}</td>
        ...
        const applyFiltersAndSort = () => {
            ...
            if (searchTerm) {
                tempOs = tempOs.filter(os =>
                    (os.servicos.map(s => s.descricao).join(', ').toLowerCase().includes(searchTerm)) ||
